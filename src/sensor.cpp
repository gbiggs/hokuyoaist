...
void
number_to_string(unsigned int num, char* dest, int length)
{
#if defined(WIN32)
...
void
Sensor::open(std::string port_options)
{
  if (verbose_) {
...
void
Sensor::set_ip(IPAddr const& addr, IPAddr const& subnet, IPAddr const& gateway)
{
...
void
Sensor::set_motor_speed(unsigned int speed)
{
  if (scip_version_ != 2) {
    throw UnknownScipVersionError();
  }
...
void
Sensor::set_high_sensitivity(bool on)
{
  if (scip_version_ != 2) {
    throw UnknownScipVersionError();
  }
...
void
Sensor::get_sensor_info(SensorInfo& info)
{
  if (scip_version_ != 2) {
    throw UnknownScipVersionError();
  }
  ...
  while (read_line_with_check(buffer, -2, true) != 0)
...
long long
Sensor::calibrate_time(unsigned int skew_sleep_time, unsigned int samples)
{
  if (verbose_) {
  ...
  for (unsigned int ii = 0; ii < samples; ii++) {
    ...
    unsigned long long laser_time = wrap_timestamp(get_timing_mode_time(&end_time)) * 1e6;
    offsets.push_back(start_time - (laser_time - (end_time - start_time) / 2));
    if (verbose_) {
  ...
  if (skew_sleep_time > 0) {
    ...
    time_skew_alpha_ = (offset2 - time_offset_) / static_cast<double>(skew_sleep_time * 1e9);
    if (verbose_) {
...
unsigned int
Sensor::get_ranges(ScanData& data, int start_step, int end_step, unsigned int cluster_count)
{
  if (scip_version_ != 2) {
    throw UnknownScipVersionError();
  }
  ...
  unsigned int cluster_count)
{
  ...
  data.laser_time_ = decode_4_byte_value(buffer) + step_to_time_offset(start_step);
  data.system_time_ = offset_timestamp(wrap_timestamp(data.laser_time_));
  // In SCIP2 mode we're going to get back 3-byte data because we're
...
unsigned int
Sensor::get_ranges_intensities_by_angle(
  ScanData& data,
  double start_angle,
  double end_angle,
  unsigned int cluster_count)
...
unsigned int
Sensor::get_new_ranges(ScanData& data, int start_step, int end_step, unsigned int cluster_count)
{
  if (scip_version_ != 2) {
    throw UnknownScipVersionError();
  }
...
unsigned int
Sensor::get_new_ranges_by_angle(
  ScanData& data,
  double start_angle,
  double end_angle,
  unsigned int cluster_count)
...
unsigned int
Sensor::get_new_ranges_intensities(
  ScanData& data,
  int start_step,
  int end_step,
  unsigned int cluster_count)
  ...
  unsigned int cluster_count)
{
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Start angle " << start_angle << " is step "
                << start_step << ", end angle " << end_angle << " is step " << end_step << '\n';
  }

  // Get the data
  return get_new_ranges_intensities(data, start_step, end_step, cluster_count);
}


double
Sensor::step_to_angle(unsigned int step)
{
...
unsigned int
Sensor::angle_to_step(double angle)
{
  unsigned int result;
  double resultF;
  resultF = front_step_ +
...
void
Sensor::clear_read_buffer()
{
  while (port_->BytesAvailableWait() > -1) {
    port_->Flush();
  }
}
...
int
Sensor::read_line(char* buffer, int expected_length)
{
...
int
Sensor::read_line_with_check(char* buffer, int expected_length, bool has_semicolon)
{
...
// the end of the data.
bool
Sensor::read_data_block(char* buffer, int& block_size)
{
  bool is_last(false);
  ...
  while (block_size < read_goal) {
    ...
    if ((bytes_read = port_->Read(&buffer[block_size], read_goal - block_size)) < 0) {
      throw ReadError(0);
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Read " << block_size << " bytes.\n";
    err_output_ << "Sensor::" << __func__ << "() Line is '" << buffer << "'\n";
  }
  ...
  confirm_checksum(buffer, bytes_to_consider, static_cast<int>(buffer[bytes_to_consider]));
...
int
Sensor::send_command(char const* cmd, char const* param, int param_length, char const* extra_ok)
{
  int statusCode = -1;
  char response[17];
  ...
  if (scip_version_ == 1) {
    ...
    response[2] = '\0';
    if (!(response[0] == '0' && response[1] == '0') && !(response[0] == '9' && response[1] == '9')) {
      if (extra_ok != 0) {
...
unsigned int
Sensor::get_timing_mode_time(unsigned long long* reception_time)
{
  ...
  unsigned int timestamp = decode_4_byte_value(&response[8]);
  return timestamp;
}
...
unsigned int
Sensor::wrap_timestamp(unsigned int timestamp)
{
...
unsigned long long
Sensor::offset_timestamp(unsigned int timestamp)
{
  return ((1 - time_drift_rate_) * timestamp * 1e6 + time_offset_) / (1 - time_skew_alpha_);
}


unsigned int
Sensor::step_to_time_offset(int start_step)
{
  if (start_step < 0) {
...
void
Sensor::find_model(char const* buffer)
{
...
void
Sensor::get_and_set_scip_version()
{
  ...
  if (scip2Failed) {
    ...
    if (verbose_) {
      err_output_ << "Sensor::" << __func__ << "() Firmware major version is " << majorVer << '\n';
    }
    ...
    if (majorVer < 3) {
      if (verbose_) {
        err_output_ << "Sensor::" << __func__ << "() Firmware does not support SCIP version 2; "
                    << "upgrade the firmware or use an older version of HokuyoAIST.\n";
      }
      throw UnknownScipVersionError();
    } else { // Otherwise we can try SCIP version 2
      port_->Flush();
      ...
      try {
        ...
        if (verbose_) {
          err_output_ << "Sensor::" << __func__
                      << "() Could not change to SCIP version 2; using SCIP " "version 1.\n";
        }
        return;
      }
      ...
      if (verbose_) {
        err_output_ << "Sensor::" << __func__ << "() Using SCIP version 2.\n";
      }
      scip_version_ = 2;
      return;
    }
...
{
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Getting default values.\n";
  }
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Got default values: " << min_angle_ << " "
                << max_angle_ << " " << resolution_ << " " << first_step_ << " " << last_step_
...
void
Sensor::process_vv_line(char const* buffer, SensorInfo& info)
{
...
void
Sensor::process_pp_line(char const* buffer, SensorInfo& info)
{
...
void
Sensor::process_ii_line(char const* buffer, SensorInfo& info)
{
  if (strncmp(buffer, "MODL", 4) == 0) {
    ...
    if (strncmp(&buffer[5], "Initial", 7) == 0) {
      ...
      if (sscanf(buffer, "SCSP:%hd%%%*4s(%d[rpm]", &info.speed_level, &info.speed) != 2) {
        throw ParseError(buffer, "Motor speed");
      }
    }
    ...
    if (strncmp(&buffer[5], "USB only", 8) == 0 || strncmp(&buffer[5], "USB Full Speed", 14) == 0) {
...
uint32_t
Sensor::process_echo_buffer(int const* buffer, int num_echos)
{
  uint32_t sum = 0;
...
void
Sensor::read_2_byte_range_data(ScanData& data, unsigned int num_steps)
{
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Reading " <<
      num_steps << " ranges.\n";
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Read " << current_step << " ranges.\n";
  }
...
void
Sensor::read_3_byte_range_data(ScanData& data, unsigned int num_steps)
{
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Reading " << num_steps << " ranges.\n";
    if (multiecho_mode_ != ME_OFF) {
      err_output_ << "Sensor::" << __func__ << "() Multi-echo mode is set to "
    ...
    for (int ii = 0; ii < numBytesInLine;) {
      ...
      if (split_count == 0) {
        ...
        if (buffer[ii] == '&') {
          ...
          data.write_range(current_step, process_echo_buffer(echo_buffer, echo_buf_ind + 1));
          if (data.ranges_) {
            if (data.ranges_[current_step] > max_range_) {
              err_output_ << "WARNING: Sensor::" << __func__ << "() Value at step " << current_step
                          << " beyond maximum range: " << data.ranges_[current_step] << '\n';
            }
          }
          current_step++;
          echo_buf_ind = 0;
      ...
      if (ii == numBytesInLine - 2) {   // Short 1 byte
        ...
        if (split_count == 1) {
          ...
          echo_buffer[echo_buf_ind] = decode_3_byte_value(split_value);
        } else if (split_count == 2) {
          split_value[1] = buffer[ii++];
          split_value[2] = buffer[ii++];
          echo_buffer[echo_buf_ind] = decode_3_byte_value(split_value);
        } else {
          echo_buffer[echo_buf_ind] = decode_3_byte_value(&buffer[ii]);
          ii += 3;
        }
...
void
Sensor::read_3_byte_range_and_intensity_data(ScanData& data, unsigned int num_steps)
{
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Reading " << num_steps
  ...
  int echo_buffer[3] = {-1, -1, -1};
  int echo_buf_ind(0); bool done(false);
  while (!done) {
    // Read a line of data
    done = read_data_block(buffer, numBytesInLine);
    // Check if we've reached the end of the data
    ...
    for (int ii = 0; ii < numBytesInLine;) {
      ...
      if (split_count == 0) {
        ...
        if (buffer[ii] == '&') {
          ...
          // Not the first value, so deal with the previous
          if (nextIsIntensity) {
            data.write_intensity(current_intensity, process_echo_buffer(echo_buffer, echo_buf_ind + 1));
          } else {
            data.write_range(current_range, process_echo_buffer(echo_buffer, echo_buf_ind + 1));
          }
          echo_buf_ind = 0;
          echo_buffer[0] = -1;
          if (data.ranges_) {
            if (data.ranges_[current_range] > max_range_ && !nextIsIntensity) {
              err_output_ << "WARNING: Sensor::" << __func__ << "() Value at step " << current_range
                          << " beyond maximum range: " << data.ranges_[current_range]
                          << " (raw bytes: ";
              if (split_count != 0) {
                err_output_ << split_value[0] << split_value[1] << split_value[2] << ")\n";
              } else {
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Read " << current_range << " ranges and "
                << current_intensity << " intensities (expected " << num_steps << ").\n";
  }
...
int
Sensor::confirm_checksum(char const* buffer, int length, int expected_sum)
{
  int checksum = 0;
  ...
  if (verbose_) {
    err_output_ << "Sensor::" << __func__ << "() Calculated checksum = " << checksum << " ("
                << static_cast<char>(checksum) << "), given checksum = " << expected_sum << " ("
                << static_cast<char> (expected_sum) << ")\n";
  }
